...
function enhanceMilestone(milestone, callback) {
    sprinter.getIssues({
        state: 'all'
      , milestone: milestone.title
        ...
        open = _.filter(issues, function(issue) {
            return issue.state == 'open';
        }).length;
        closed = _.filter(issues, function(issue) {
        ...
        _.each(milestoneList, function(milestone) {
            if (milestone.description) {
                enhancers.push(function(localCallback) {
                    enhanceMilestone(milestone, function(err, enhanced) {
                        if (err) {
...
function populateSuperIssues(milestones, callback) {
    var fetchers = [];
    ...
    async.parallel(fetchers, function(err, issuesList) {
        ...
        _.each(issuesList, function(issues, index) {
            var milestoneIndex = Math.floor(index / 2)
              , sortedIssues = _.sortBy(issues, function(issue) {
                    return issue.state
                }).reverse();
            // Even index results are super issue lists, odd index results are
            // stand-alone issues lists.
            if (index % 2 == 0) {
                ...
                milestones[milestoneIndex].superIssues = sortedIssues;
            } else {
                milestones[milestoneIndex].standAloneIssues = sortedIssues;
            }
        });
    ...
    sprinter.getMilestones(function(err, milestones) {
        ...
        consolidateMilestones(milestones,
            function(err, consolidatedMilestones) {
                ...
                populateSuperIssues(consolidatedMilestones, 
                    function(err, milestonesWithIssues) {
                        var sortedMilestones = _.sortBy(milestonesWithIssues,
                            function(ms) {
                                return ms.title;
                            }
                        );
