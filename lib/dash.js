...
SprinterDash.prototype._prepareForStartup = function(staticDir) {
    ...
    this._roadMap = RoadMap({
        repos: this.repos
      , travisOrg: this._config.travisOrg
      , ghUsername: this._config.ghUsername
      , ghPassword: this._config.ghPassword
      , staticDir: staticDir + '/'
    });
};
...
SprinterDash.prototype.showRoadMap = function() {
    return this._showView('roadmap', function(req) {
        return {
            title: 'Road Map'
        };
    });
...
SprinterDash.prototype.attach = function(app, urlPrefixIn) {
    var urlPrefix = resolveUrlPrefix(urlPrefixIn)
      ...
      , roadMapRoute = urlPrefix + 'roadmap'
      , roadMapDataRoute = urlPrefix + '_roadmap'
      , staticUrl = urlPrefix + staticDir;

    this._urlPrefix = urlPrefix;

    this._prepareForStartup(staticUrl);
    ...
    app.get(roadMapRoute, this.showRoadMap());
    app.get(roadMapDataRoute, this._roadMap);
